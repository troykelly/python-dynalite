...
INITIAL_RETRY_DELAY = 1        # if a request for channel level didn't get acknowledge, when to retry (subsequent retries will be 2x, 4x, 8x, etc.)
STARTUP_RETRY_DELAY = 60       # initial retry timeout for the beginning, since it could take time to settle on Dynet for large environments
MAXIMUM_RETRY_DELAY = 60*60    # Minimal retry frequency in seconds - default 1 hour, so if something never answers, it will be pinged once an hour XXX consider changing
